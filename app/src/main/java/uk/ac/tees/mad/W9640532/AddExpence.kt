...
class AddExpence : AppCompatActivity() {
    ...
    private fun handleAddExpense() {
        if (expenseTypeGroup.checkedRadioButtonId == R.id.oneTimeExpense) {
            ...
            // Store one-time expense data
            saveExpense("One-Time", amount, category, "", "")
        } else {
            val amount = amountInput.text.toString()
            val startMonth = startMonthInput.text.toString()
            val endMonth = endMonthInput.text.toString()
            // Store monthly expense data
            saveExpense("Monthly", amount, "", startMonth, endMonth)
        }
    ...
    private fun saveExpense(type: String, amount: String, category: String, startMonth: String, endMonth: String) {
        val userId = FirebaseAuth.getInstance().currentUser?.uid
        val database = FirebaseDatabase.getInstance().reference

        if (userId != null) {
            val expense = Expense(type, amount, category, startMonth, endMonth, photoBase64,"expense")
            // Use the 'expenses' node under the user's ID, then create a unique key for each new expense
